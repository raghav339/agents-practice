...
def run_agent(user_query,verbose=True,max_iterations=10,n=4):
    tool_desc = "\n".join(
    ...
    for i in range(max_iterations):
        if((i)==n):
            # 1. Only summarize actual successful exchanges, not failures
            total_content = ""
            for msg in conversation:
                if msg["role"] == "system":
                    continue
                if "Invalid JSON" in msg.get("content", ""):
                    continue  # skip failure retries
                total_content += f'\n{msg["content"]}'

            # 2. Increase n for complex multi-step tasks
            # n=3 is too aggressive — use n=6 or 7 minimum
            SYS_PROMPT="""
                You are a conversation compressor. Your ONLY job is to summarize what has happened so far.
                DO NOT answer any questions. DO NOT continue any tasks.

                Compress the conversation into:
                - What the user originally asked
                - What tools were called and their EXACT results (preserve key facts, numbers, summaries)
                - What steps are still pending

                IMPORTANT: Tool results contain critical information. Always include the actual data 
                returned by tools, not just that a tool was called.

                IMPORTANT: Only report tools that were ACTUALLY called with ACTUAL results 
                from Observation: entries. Do NOT infer or assume any tool was called unless 
                you see an explicit "Observation:" in the conversation.
            ...
            messages=[
                ...
                {"role":"user" ,"content":f" Give me a summary of:\n {total_content} and give summary in string"}]
           )
            print(f"Summary : {res.choices[0].message.content}")
            print("SUMMARY ENDS------------------------------------------")
            conversation=[
                {"role":"system","content":system},
                {"role":"user","content":f"Context so far: {res.choices[0].message.content}\n\nOriginal task: {user_query}"}
            ]
            checked_plans = False
